...
class Home_controller extends Home_Core_Controller
{
    ...
    public function index()
    {
        ...
        $data['latest_ads'] = $this->latest_adcategory_posts;
    ...
    public function classifieds()
    {
        ...
        $data['adcategories'] = $this->adcategory_model->get_parent_adcategories();
        $data['latest_ads'] = $this->latest_adcategory_posts;
        $data['featured_posts'] = $this->classified_model->get_featured_posts();


        $this->load->view('partials/_header', $data);
        $this->load->view('classifieds', $data);
        $this->load->view('partials/_footer', $data);
    }
     ...
     * Ad Category
     */
    public function adcategory($adcategory_slug)
    {
        get_method();
        $adcategory_slug = clean_slug($adcategory_slug);
        $category = $this->adcategory_model->get_category_by_slug($adcategory_slug);
        // //check tag exists
        if (empty($category)) {
        ...
        $data['adcategories'] = $this->adcategory_model->get_adcategories();
        ...
        if (empty($total_rows)) {
            $total_rows = $this->classified_model->get_post_count_by_category($category->id);
        ...
        if (empty($data['posts'])) {
            $data['posts'] = $this->classified_model->get_paginated_category_posts($category->id, $pagination['offset'], $pagination['per_page']);
            set_cache_data($posts_key . '_page' . $pagination['current_page'], $data['posts']);
        }

        $this->load->view('partials/_header', $data);
        $this->load->view('adcategory', $data);
        $this->load->view('partials/_footer');
    }
     ...
     * Ad Detail 
     */
    public function ad_details($ad_slug)
    {
        get_method();
        $ad_slug = clean_slug($ad_slug);
        if (empty($ad_slug)) {
            redirect(lang_base_url());
        }
        $ad_details = $this->classified_model->get_post($ad_slug);

        $data['title'] = $ad_details->title;
        $data['description'] = $ad_details->summary;
        $data['keywords'] = $ad_details->keywords;
        $data['ad_detail'] = $this->classified_model->get_post($ad_slug);
        $data['adcategories'] = $this->adcategory_model->get_adcategories();
        //print_r($data['ad_detail']);
        $data['ad_comments'] = $this->classified_model->get_comments($data['ad_detail']->id,10);
        //print_r($data['ad_comments']);
        $this->load->view('partials/_header', $data);
        $this->load->view('ad', $data);
        $this->load->view('partials/_footer');

        $this->classified_model->increase_post_pageviews($ad_details);
    }
    ...
    public function add_to_favourite()
    {
        ...
        $last_id = $this->classified_model->add_to_favourie_ad($ad_id,$user_id);

        if($last_id != "" || $last_id != null || $last_id > 0){
            $data = array(
                'result' => 1,
                'html_content' => 'Ad successfully added to favourites.'
            );
        }else{
            $data = array(
                'result' => 0,
    ...
    public function remove_from_favourite()
    {
        ...
        $last_id = $this->classified_model->remove_from_favourite_ad($ad_id,$user_id);

        if($last_id){
            $data = array(
                'result' => 1,
                'html_content' => 'Ad successfully removed from favourites.'
            );
        }else{
            $data = array(
                'result' => 0,
                'html_content' => "Something went wrong. Please try again.",
            );
        }           
     ...
     * Contact Page Post
     */
    public function add_comment_on_classified_ad()
    {
        post_method();
        //validate inputs
        $this->form_validation->set_rules('authorName', 'authorName', 'required|max_length[200]');
        $this->form_validation->set_rules('authorEmail', 'authorEmail', 'required|max_length[200]');
        $this->form_validation->set_rules('comment', 'comment', 'required|max_length[5000]');
        if ($this->form_validation->run() === FALSE) {
            $this->session->set_flashdata('errors', validation_errors());
            $this->session->set_flashdata('form_data', $this->classified_model->comment_input_values());
            redirect($this->agent->referrer());
        } else {           
            if ($this->classified_model->add_comment_on_add()) {
                $this->session->set_flashdata('success', trans("message_contact_success"));
                redirect($this->agent->referrer());
            } else {
                $this->session->set_flashdata('form_data', $this->classified_model->comment_input_values());
    ...
    public function contact_seller_of_ad(){
        post_method();
        //validate inputs
        $this->form_validation->set_rules('authorName', 'authorName', 'required|max_length[200]');
        $this->form_validation->set_rules('authorEmail', 'authorEmail', 'required|max_length[200]');
        ...
        if ($this->form_validation->run() === FALSE) {
            $this->session->set_flashdata('errors', validation_errors());
            $this->session->set_flashdata('form_data', $this->classified_model->seller_contact_input_values());
            redirect($this->agent->referrer());
        } else {           
            if ($this->classified_model->seller_contact_on_ad()) {
                $this->session->set_flashdata('success', trans("message_contact_success"));
                redirect($this->agent->referrer());
            } else {
                $this->session->set_flashdata('form_data', $this->classified_model->seller_contact_input_values());
    ...
    public function get_all_my_listing()
    {
        get_method();
        //check user
        if (!auth_check()) {
        ...
        $total_rows = get_cached_data($count_key);
        if (empty($total_rows)) {
            $total_rows = $this->classified_model->get_post_count_by_user($data['user']->id);
            set_cache_data($count_key, $total_rows);
        }
        //set paginated
        ...
        $data['posts'] = get_cached_data($posts_key . '_page' . $pagination['current_page']);
        if (empty($data['posts'])) {
            $data['posts'] = $this->classified_model->get_paginated_user_posts($data["user"]->id, $pagination['offset'], $pagination['per_page']);
            set_cache_data($posts_key . '_page' . $pagination['current_page'], $data['posts']);
        }
        ...
        $this->load->view('partials/_header', $data);
        $this->load->view('profile/mylistings', $data);
        $this->load->view('partials/_footer');
    }

    public function get_all_my_favourite_listings()
    {
        get_method();
        //check user
        if (!auth_check()) {
        ...
        $data['posts'] = $this->classified_model->get_all_favourites($data["user"]->id);
        ...
        $this->load->view('partials/_header', $data);
        $this->load->view('profile/favourites', $data);
        $this->load->view('partials/_footer');
    }
    ...
    public function get_all_inquiries()
    {
        ...
        $data['inquiries'] = $this->classified_model->get_all_inquiries($data["user"]->id);
        ...
        $this->load->view('partials/_header', $data);
        $this->load->view('profile/inquiries', $data);
        $this->load->view('partials/_footer');
    }
}
