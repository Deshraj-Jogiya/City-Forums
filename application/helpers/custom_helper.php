...
//check user favourited ad
if (!function_exists('is_user_favourite_ad')) {
    function is_user_favourite_ad($user_id, $ad_id)
    {
        $ci =& get_instance();
        return $ci->classified_model->is_added_to_favourite($user_id, $ad_id);
    }
}


//get subcomments
if (!function_exists('get_total_ads_by_category_id')) {
    function get_total_ads_by_category_id($category_id)
    {
        // Get a reference to the controller object
        $ci =& get_instance();
        return $ci->adcategory_model->get_total_ads_by_adcategory($category_id);
    }
}
?>
