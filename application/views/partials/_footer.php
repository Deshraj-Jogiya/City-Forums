...
$(document).on('click', '.add-to-favorite', function (e) {
    ...
    if(user_id == ""){
        document.getElementById("inner-message").innerHTML = "Please login to your account.";
        $("#message").show();
        setTimeout(function () {
            $("#message").hide(); 
            // $('#modal-login').show();                 
        ...
        $.ajax({
            type: "POST",
            url: base_url + "home_controller/add_to_favourite",
            data: data,
            success: function (response) {
                var obj = JSON.parse(response);
                if (obj.result == 1) {
                    document.getElementById("inner-message").innerHTML = obj.html_content;
                    $("#message").show();
                    $('.save-ad-favourite a').removeClass('add-to-favorite');
                    $('.save-ad-favourite a').addClass('remove-from-favorite');
                    $('.save-ad-favourite a').html('<i class="fas fa-heart"></i> <span>Saved</span>');
                    setTimeout(function () {
                       $("#message").hide();                   
                    }, 3000)
                }
...
$(document).on('click', '.remove-from-favorite', function (e) {
    ...
    if(user_id == ""){
        ...
        $.ajax({
            type: "POST",
            url: base_url + "home_controller/remove_from_favourite",
            data: data,
            success: function (response) {
                var obj = JSON.parse(response);
                if (obj.result == 1) {
                    document.getElementById("inner-message").innerHTML = obj.html_content;
                    $("#message").show();
                    $('.save-ad-favourite a').removeClass('remove-from-favorite');
                    $('.save-ad-favourite a').addClass('add-to-favorite');
                    $('.save-ad-favourite a').html('<i class="fas fa-heart"></i> <span>Save to favourite</span>');
                    setTimeout(function () {
                       $("#message").hide();                   
                    }, 3000)
                }
